...
getCoreFromBGENV() {
	...
	#DEPLOYMENTS=$(curl -sS "https://eu1.anypoint.mulesoft.com/amc/application-manager/api/v2/organizations/${ORG_ID}/environments/${ENV_ID}/deployments" \
  #  -H "Authorization: Bearer $TOKEN")

 	curl_func "https://eu1.anypoint.mulesoft.com/amc/application-manager/api/v2/organizations/${ORG_ID}/environments/${ENV_ID}/deployments" $TOKEN
 	if [[ $STATUS_CODE != 200  ]] ; then
	...
	for indexA in ${!IDS[@]};
	do
		# APP_DEPLOYMENT=$(curl -sS "https://eu1.anypoint.mulesoft.com/amc/application-manager/api/v2/organizations/${ORG_ID}/environments/${ENV_ID}/deployments/${IDS[$indexA]}" \
	 #  			-H "Authorization: Bearer $TOKEN")
	  curl_func "https://eu1.anypoint.mulesoft.com/amc/application-manager/api/v2/organizations/${ORG_ID}/environments/${ENV_ID}/deployments/${IDS[$indexA]}" $TOKEN
...
then 
	...
	TOKEN=$(curl -sS  -X POST https://eu1.anypoint.mulesoft.com/accounts/login -H 'Content-Type: application/json' \
...
# ORG_INFO=$(curl -sS -X GET https://eu1.anypoint.mulesoft.com/accounts/api/me -H "Authorization: Bearer $TOKEN")
curl_func "https://eu1.anypoint.mulesoft.com/accounts/api/me" $TOKEN
...
do 
	#ENV_INFO=$(curl -sS -X GET "https://eu1.anypoint.mulesoft.com/accounts/api/organizations/${ORG_IDS[$index]}/environments" -H "Authorization: Bearer $TOKEN")
	curl_func "https://eu1.anypoint.mulesoft.com/accounts/api/organizations/${ORG_IDS[$index]}/environments" $TOKEN
